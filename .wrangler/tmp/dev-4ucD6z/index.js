...
// .wrangler/tmp/bundle-6AP9nM/checked-fetch.js
...
function parseEngineIOPackets(data) {
  ...
  while (i < data.length) {
    ...
    if (lengthEnd === -1) {
      if (i < data.length) {
        const type2 = data[i];
        const payload2 = data.substring(i + 1);
        packets.push({ type: type2, payload: payload2 });
      }
      break;
    }
...
function handleSocketIOEvent(sessionId, session, eventData) {
  ...
  switch (eventName) {
    ...
    case "leaveRoom":
      if (session.roomId) {
        const room2 = rooms.get(session.roomId);
        if (room2) {
          room2.players = room2.players.filter((p) => p !== sessionId);
        ...
        session.roomId = null;
        const leaveResponse = ENGINE_IO_PACKETS.MESSAGE + SOCKET_IO_PACKETS.EVENT + JSON.stringify(["leftRoom", {}]);
...
// .wrangler/tmp/bundle-6AP9nM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6AP9nM/middleware-loader.entry.ts
